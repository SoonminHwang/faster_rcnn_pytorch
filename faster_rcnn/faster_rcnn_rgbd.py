...
class RPN_RGBD2(nn.Module):
    _feat_stride = [16, ]
    # anchor_scales = [8, 16, 32]

    def __init__(self):
        super(RPN_RGBD2, self).__init__()

        self.features = VGG16_RGBD2(bn=False)
    ...
    def forward(self, im_data, depth_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        im_data = network.np_to_variable(im_data, is_cuda=True)
        im_data = im_data.permute(0, 3, 1, 2)

        depth_data = network.np_to_variable(depth_data, is_cuda=True)
        depth_data = depth_data.permute(0, 3, 1, 2)

        features = self.features(im_data, depth_data)
...
class FasterRCNN_RGBD2(nn.Module):
    n_classes = 21
    ...
    def __init__(self, classes=None, debug=False):
        super(FasterRCNN_RGBD2, self).__init__()
    ...
    def forward(self, im_data, depth_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        features, rois = self.rpn(im_data, depth_data, im_info, gt_boxes, gt_ishard, dontcare_areas)
