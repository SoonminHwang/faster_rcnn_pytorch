...
class VGG16_RGBD2(nn.Module):
    def __init__(self, bn=False):
        super(VGG16_RGBD2, self).__init__()
        ...
        self.conv1_d = nn.Sequential(Conv2d(1, 64, 3, same_padding=True, bn=bn),
                                   Conv2d(64, 64, 3, same_padding=True, bn=bn),
                                   nn.MaxPool2d(2))
        self.conv2_d = nn.Sequential(Conv2d(64, 128, 3, same_padding=True, bn=bn),
                                   Conv2d(128, 128, 3, same_padding=True, bn=bn),
                                   nn.MaxPool2d(2))

        self.conv3_concat = Conv2d(256, 128, 3, same_padding=True, bn=bn)
    ...
    def forward(self, im_data, depth_data):
        # im_data, im_scales = get_blobs(image)
        # im_info = np.array(
        #     [[im_data.shape[1], im_data.shape[2], im_scales[0]]],
        #     dtype=np.float32)
        ...
        y = self.conv1_d(depth_data)
        y = self.conv2_d(y)

        try:
          z = self.conv3_concat( torch.cat([x, y], 1) )
        except:
          import ipdb
          ipdb.set_trace()
